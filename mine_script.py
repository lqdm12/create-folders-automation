...
# Função para criar as pastas recursivamente
def criar_pastas(base_path, materias):
    if not os.path.exists(base_path):
        os.makedirs(base_path)
    
    for materia, submaterias in materias.items():
        materia_path = os.path.join(base_path, materia)
        if not os.path.exists(materia_path):
            os.makedirs(materia_path)
        
        # Recursivamente cria as pastas para os tópicos e sub-tópicos
        if isinstance(submaterias, dict):
            criar_pastas(materia_path, submaterias)
        elif isinstance(submaterias, list):
            for subpasta in submaterias:
                subpasta_path = os.path.join(materia_path, subpasta)
